...
namespace BookTradesProject.Controllers
{
    public class TrocasController : Controller
    {
        ...
        public async Task<IActionResult> Create([Bind("Id,LivroDado")] Trocas trocas)
        {
            var vendedor = User.FindFirstValue(ClaimTypes.NameIdentifier);
            trocas.Vendedor = vendedor;
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(int id, [Bind("Id,LivroDado,LivroRecebido,Vendedor,Comprador,Estado,Timestamp")] Trocas trocas)
        {
            if (id != trocas.Id)
            {
                return NotFound();
